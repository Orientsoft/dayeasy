...
using TransactionalBehavior = DayEasy.Core.Domain.TransactionalBehavior;
...
namespace DayEasy.EntityFramework
{
    public class CodeFirstDbContext : DbContext, IUnitOfWork
    {
        ...
        protected static string GetConnectionStringName(string key = "DayEasy-ConnectionStringName",
            string def = "default")
        {
        ...
        {
            get { return ConfigHelper.GetAppSetting(defaultValue: false, supressKey: "DayEasy-DataLoggingEnabled"); }
        }
        ...
        private void ApplyConcepts()
        {
            ...
            {
                if (!_entityStates.ContainsKey(entry))
                    continue;
        ...
        public int Transaction(Action<IUnitOfWork> action)
        {
            IsTransaction = true;
            action(this);
            return SaveChanges();
