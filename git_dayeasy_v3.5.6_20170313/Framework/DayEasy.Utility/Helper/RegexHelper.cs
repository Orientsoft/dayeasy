...
namespace DayEasy.Utility.Helper
{
    ...
    public class RegexHelper
    {
        ...
        private const string MobileRegex = @"^1[3456789]\d{9}$";
        ...
        /// <summary> 邮件昵称 </summary>
        public static string EmailNick(string email)
        {
            if (email.IsNullOrEmpty())
                return email;
            return email.As<IRegex>().Replace("^([\\w\\W]{3})[^@]*(@[\\w\\W]+)$", "$1***$2");
        }

        /// <summary> 手机号码昵称 </summary>
        public static string MobileNick(string mobile)
        {
            if (mobile.IsNullOrEmpty())
                return mobile;
            return mobile.As<IRegex>().Replace("^([0-9]{3})[0-9]+([0-9]{4})$", "$1****$2");
        }
    }
}
