var errorBook = (function () {
    var EB = {
        ...
        bind: function () {
            ...
            //错题列表
            var html = template('eb-item', {list: EB.list, source_type: EB.params.source_type});
            $(".q-list").html(html).show();
            //解析并展示公式
...
$(function ($) {
    ...
    $.get("errorBook/subjects").success(function (json) {
        ...
        if (json && json.length) {
            ...
            for (var i = 0; i < json.length; i++) {
                $subjects.append('<div class="item" data-val="' + json[i].key + '">' + json[i].value + '</div>');
            }
        }
    });
    /*****初始化页面*****/
    var passStatus = {'-1': '全部错题', '1': '已过关', '0': '未过关'};
    //收藏筛选
    $(".sn-collect").delegate(".item", "click", function (e) {
        e.stopPropagation();
        var $this = $(this),
            pass = $this.data("val");
        if (errorBook.params.pass == pass) {
        ...
        $this.parent().hide();
        $this.data('val', errorBook.params.pass).text(passStatus[errorBook.params.pass]);
        errorBook.params.pass = pass;
    ...
    $(".sn-source").delegate(".item", "click", function (e) {
        ...
        if (singer.isUndefined(val))
            return;
        if (val == errorBook.params.source_type) {
            $this.parents(".sing").data("open", 0).removeClass("active");
            $this.parent().hide();
            return;
        }
    ...
    $(".sn-subjects").delegate(".item", "click", function (e) {
        ...
        $types.html("");
        if (val > -1) {
            $.get("errorBook/question-type?subjectId=" + val).success(function (json) {
                if (json.status && json.data && json.data.length) {
                    $types.append('<div class="item" data-val="-1">不限</div>');
                    for (var i = 0; i < json.data.length; i++) {
                        $types.append('<div class="item" data-val="' + json.data[i].id + '">' + json.data[i].name + '</div>');
                    }
                }
            });
        } else {
    ...
    function searchKeys() {
        ...
        skLock = true;
        $.post("errorBook/search-keys", {key: val, subjectId: errorBook.params.subject}, function (json) {
            if (json.status) {
                lastKey = val;
    ...
    $(".q-list").delegate(".span-pass", "click", function () {
        //过关、取消过关
        var $t = $(this);
        var id = $t.parents(".q-item").data("id");
        var pass = !($t.data("pass") == 1);
        $.post("errorBook/pass", {id: id, isPass: pass}, function (json) {
            if (json.status) {
                if (pass) {
                    $t.data("pass", 1).html('<i class="iconfont dy-icon-48"></i> 已过关');
                } else {
                    $t.data("pass", 0).html('<i class="iconfont dy-icon-five-star"></i> 过关');
                }
                if (errorBook.params.is_collect) {
                    var $total = $(".e-total");
    ...
    }).delegate(".span-dw", "click", function () {
        //下载
 ...
 * 根据缓存 显示加入下载、移除下载
 */
template.helper("showDowloadHtml", function (id) {
    if (errorBook.dw.check(id)) {
        return '<span class="oi collection-icon span-dw dw-active"><i class="fa fa-times"></i> 移除下载</span>&nbsp;&nbsp;';
    }
    return '<span class="oi collection-icon span-dw"><i class="fa fa-plus"></i> 加入下载</span>&nbsp;&nbsp;';
});