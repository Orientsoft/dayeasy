...
var reason = (function () {
    var R = {
         ...
         */
        load: function (data, box, callback, existsCallBack, params) {
            R.data = data;
            R.box = box || R.box;
            if (!R.templateLoaded) {
                singer.loadTemplate('reason-edit-v3', function () {
        ...
        getData: function (callback, existsCallBack, params) {
            $.post("/reason/load",
                {id: R.data.eid, question_id: R.data.qid, batch: R.data.batch},
                function (json) {
                    if (json.status) {
                        var data = json.data;
        ...
        init: function (callback) {
            ...
            var tags = singer.tags({
                ...
                change: function (data) {
                    ...
                    var $sysTags = $box.find('.sys-tags span.selected');
                    for (var i = 0; i < $sysTags.length; i++) {
                        var $sysTag = $sysTags.eq(i),
                            tag = $sysTag.html();
                        if (!singer.inArray(tag, data)) {
                            $sysTag.removeClass('selected');
                        }
                    }
                }
            });
