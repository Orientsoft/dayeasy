...
(function ($, undefined){
    ...
    $.fn.extend({
        chart: function (options){
            ...
            if(isVertical){
                opts.chartWidth ? opts.chartWidth : opts.chartWidth = Math.min((opts.width - 93) / opts.data.length, 70);
            } else {
                opts.chartWidth ? opts.chartWidth : opts.chartWidth = Math.min((opts.height) / opts.data.length, 70);
            }
            ...
            var step = 6;
            if(max < 100 || max % 50 == 0 || max % 6 != 0) step = 5;
            if(isVertical){
            ...
            for (var i = 0; i < step; i++) {
                var $step = $('<div class="d-chart-step">'),
                    stepValue = parseFloat(((i + 1) * (max / step)).toFixed(2));
                if(isVertical){
                    $step.css({
            ...
            if(opts.showAverage && opts.average > 0){
            ...
            $wrap.append($chart);
            if(opts.xTitle){
                $wrap.append('<div class="d-chart-x">' + opts.xTitle + '</div>');
            }
            if(opts.yTitle){
                $wrap.append('<div class="d-chart-y">' + opts.yTitle + '</div>');
            }
            $t.append($wrap);
