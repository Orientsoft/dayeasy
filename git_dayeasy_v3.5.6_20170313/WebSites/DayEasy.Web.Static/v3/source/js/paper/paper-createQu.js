$(function () {
    var stage = $("#stage").val();
    var selector = $("#kPoints").tokenInput('/sys/knowledges?stage=' + stage, {
        method: "POST",
        queryParam: "keyword",
        hintText: "",
    ...
    $(".f-success").click(function () {
        var qtype = parseInt($("#selectedQtype").val());
        ...
        var showSmall = window.getSmallQtype();
        if (isNaN(smallQuNum)) {
            smallQuNum = 0;
        }

        if ($.inArray(qtype, showSmall) > -1 && optionNum > 0 && smallQuNum < 1) {
            singer.msg("请输入小问数！");
            return false;
        }
        ...
        $.post($("#saveQuUrl").val(), {
            ...
            if (res.Status) {
                var paperInfo = window.GetPaperInfo();
                if (paperInfo) {
                    var qitem = {};
                    qitem.QId = res.Data;
                    qitem.Type = qtype;
                ...
                var form = $('<form></form>');
                //var url = window.location.href + "#actionDiv_" + atype;
                window.location.hash = 'actionDiv_' + atype;
    ...
    $(".f-qtype").click(function () {
        ...
        window.showOptionAndSmallQu(value);
    });
...
var showOptionAndSmallQu = function (type) {
    var showOption = window.getOptionQtype();
    var showSmall = window.getSmallQtype();

    $("#optionDiv").addClass("hide").find('select').val(0);
    ...
    if (type == 1) {
        $("#optionDiv").find('select').val(4);
    } else {
        $("#optionDiv").find('select').val(0);
    }
};
...
var getOptionQtype = function () {
    var optionStr = $("#hasOptionQType").val();

    var options = eval("(" + optionStr + ")");

    return options;
};

var getSmallQtype = function () {
    var smallStr = $("#hasSmallQType").val();

    var smalls = eval("(" + smallStr + ")");
