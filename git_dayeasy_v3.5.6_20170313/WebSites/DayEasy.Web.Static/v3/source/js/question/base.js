...
(function (S, $) {
    ...
    S._mix(S, {
        ...
        ranges: [{id: 0, name: '自己'}, {id: 2, name: '全网'}],
...
//            body = body.replace(/&nbsp;/gi, " ");
            ...
            var recognitionDetails = function (str) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        if (singer.inArray(styles[2], tags) && options.length == 2 && i > 0) {
                            ...
                            if (str.indexOf(tagWords.option) >= 0) {
                                details.push({
                                    body: S.trimTrn(item),
                                    images: [],
                                    sort: sort
        ...
        textEditor: function (item, callback) {
            ...
            if (!$editor || !$editor.length) {
                $editor = $('<script id="dy-editor" type="text/plain"></script>');
                $("body").append($editor);
            }
            ue = UE.getEditor("dy-editor", {
                toolbars: [
                    ['source', '|', 'Undo', 'Redo', '|', 'bold', 'italic', 'underline', '|', 'justifyleft', 'justifyright', 'justifycenter', 'justifyjustify', '|',
                        'inserttable', 'deletetable', 'kityformula', 'spechars', 'superscript', 'subscript']
                ],
