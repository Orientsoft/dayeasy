...
var logger = singer.getLogger("modifyCtrl"),
    modifyCtrl = ["$scope", "$http", "$sce", function ($scope, $http, $sce) {
        ...
        var load = function () {
            ...
            $http.post('/sys/qtype/' + $scope.question.type).success(function (json) {
            ...
            var selector = $("#kPoints").tokenInput('/sys/knowledges?stage=' + $scope.question.stage, {
                method: "POST",
                queryParam: "keyword",
                hintText: "请输入知识点关键字",
                noResultsText: "没有找到相关知识点",
                searchingText: "查询中，请稍候...",
                placeholder: "输入知识点关键字",
                tokenLimit: 5,
                propertyToSearch: 'path',
                excludeCurrent: false,
                prePopulate: $scope.question.knowledges,
                tokenFormatter: function (item) {
                    var string = item['name'];
                    return "<li><p>" + string + "</p></li>";
                },
                onAdd: function () {
                    $scope.question.knowledges = selector.tokenInput("get");
                    $scope.$digest();
                },
                onDelete: function () {
                    $scope.question.knowledges = selector.tokenInput("get");
...
//        });
        ...
        $scope.save = function (type) {
            ...
            var saveSubmit = function () {
                ...
                //logger.info(data);
                var method = $scope.saveAs ? 'save-as' : 'save';
            ...
            var saveKnowledge = function () {
                $http.post('/question/save-knowledge', {
                    id: data.id,
                    knowledges: encodeURIComponent(singer.json(data.knowledges))
                }).success(function (json) {
                    if (json.status) {
                        singer.alert('保存成功！');
                    } else {
