(function ($) {
    ...
    paperMaker.prototype = {
        ...
        showSaveDiv: function (isTemp) {
            ...
            var d = dialog({
                ...
                ok: function () {
                    ...
                    $.post(saveUrl, { isTemp: isTemp, paperId: paperId, paperData: jsonDataStr, shareSchool: shareSchool, answers: answers }, function (res) {
                        if (res.Status) {
                            d.close().remove();
        ...
        showAddQuestion: function () {
            var $createQu = $(this).parent('div').next('div.fa-createQuDiv');
            if ($(this).children('i').hasClass("fa-plus")) {
                $('div.fa-createQuDiv').hide();
                $('div.fa-createQuDiv').children('div').empty();
                $('div.fa-createQuDiv').children('span').removeClass('hide');
                $(".f-createQu").children('i').addClass("fa-plus").removeClass('fa-times');
                //处理当前点击
                $(this).children('i').addClass("fa-times").removeClass("fa-plus");
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        $createQu.show().children('div').empty().append(res);
                        $createQu.children('span').addClass('hide');
                    }
                });
            } else {
                $(this).children('i').removeClass("fa-times").addClass("fa-plus");
                $('div.fa-createQuDiv').children('div').empty();
                $createQu.hide();
            }
        },
