...
var logger = singer.getLogger("addCtrl"),
    ...
    addCtrl = ['$scope', '$http', '$sce', function ($scope, $http, $sce) {
        ...
        var url = (singer.sites.main || '') + '/sys',
            ...
            currentType,//当前题型
            loader = (function () {
                $http.post(url + '/question_init')
                    .success(function (json) {
                        $scope.qt.subject_id = json.subject_id;
        ...
        var selector = $("#kPoints").tokenInput($("#getKpUrl").val(), {
            ...
            onAdd: function () {
                $scope.qt.points = selector.tokenInput("get");
                $scope.$digest();
            },
            onDelete: function () {
                $scope.qt.points = selector.tokenInput("get");
        ...
        $scope.save = function () {
            ...
            $scope.saving = true;
            //logger.info(data);
            $http.post('save', data).success(function (json) {
                if (json.status) {
